    ...
    <div class="section1" :class="{ bgnight: isNight }">
      ...
      <div class="lang">
        ...
        <div class="menu-lang" :class="{ show: showLang }">
          ...
          <div @click="saveLocale('en')" @click.stop="showLang = false">
            <input type="radio" id="en" value="en" v-model="$i18n.locale" />EN
          </div>
        </div>
      </div>
      <div class="overlay" :class="{ show }" @click="showMenu"></div>
      ...
      >
        <router-link
          ...
          class="menu-link"
        >
          <div class="menu-item">{{ $t(item.name) }}</div>
        </router-link>
        <div class="menu-lang">
    ...
    <div class="sub-section1">
      ...
      <div class="tag-toggle" v-if="isNight">
        <div class="d-flex">
          ...
          <span class="dayTime" @click="bgToggle(!isNight)">Day Time</span>
        </div>
      </div>

      <div class="tag-toggle-reverse" v-if="!isNight">
        <div class="d-flex">
          <span class="nightTime" @click="bgToggle(!isNight)">Night Time</span>
          <div class="allTags">
            <div class="tag"></div>
            <div class="tags"></div>
          </div>
...
export default {
  setup() {
    const state = reactive({
      ...
      lang: localStorage.getItem("locale") || process.env.VUE_APP_I18N_LOCALE,
      showLang: false,
    });
